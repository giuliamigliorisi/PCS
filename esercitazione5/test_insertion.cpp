...
int main(void)
{
	...
	for (int i=0; i<100; i++)
	{
		...
		std::cout << "vettore ordinato (test insertion) " << i+1 << " lunghezza " << dimensione_scelta << ": ";
		print_vector(v);
	...
	insertion_sort(stringa, 0, stringa.size()-1);
	if (not is_sorted(stringa)) {
			return EXIT_FAILURE;
		}
		
	std::cout << "vettore ordinato (test insertion): ";
	print_vector(stringa);
